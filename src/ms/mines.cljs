...
(defn reveal-all-x
  ...
  [board model pos op]
  (let [size (count board)
        xs (map (fn [y] (map (fn [x] (if (and (= mine-s (get-pos model [x y]))
                                              (not (get-pos board [x y])))
                                        [x y]))
                             (range size)))
                (range size))]
    (loop [b (put-in-board board pos mine-s)
           xs (vec (filter identity (apply concat xs)))]
      (if (empty? xs)
        b
        (recur (reveal b model (first xs) op) (rest xs))))))
